...
export default function ResponseDisplay({ response, responseRef }) {
  ...
  const formatContent = (content) => {
    ...
    return lines.map((line, index) => {
      ...
      if (trimmed.startsWith('-') || trimmed.startsWith('•') || trimmed.startsWith('*') || 
        ...
        return (
          <div key={index} className="flex items-start space-x-3 mb-3 p-3 bg-blue-50/50 rounded-lg border-l-4 border-blue-300">
      ...
      if (trimmed.endsWith(':') || (trimmed.includes('**') && trimmed.startsWith('**'))) {
        return (
          <h4 key={index} className="text-lg font-bold text-gray-800 mt-6 mb-3 flex items-center">
      ...
      if (trimmed.includes('month') || trimmed.includes('Month')) {
        ...
        return (
          <div key={index} className="bg-gradient-to-r from-green-50 to-blue-50 p-4 rounded-xl border border-green-200 mb-4">
            <div className="flex items-center space-x-2 mb-2">
              <span className="bg-green-500 text-white px-3 py-1 rounded-full text-sm font-bold">
                {monthMatch && monthMatch[1] ? monthMatch[1] : '?'}M
              </span>
              <h5 className="font-bold text-gray-800">Milestone</h5>
            </div>
            <p className="text-gray-700">{trimmed}</p>
          </div>
        );
      }
  ...
  return (
    <div ref={responseRef} className="mt-8 bg-white rounded-2xl shadow-2xl border border-gray-200 overflow-hidden">
      {/* Header */}
      <div className="bg-gradient-to-r from-blue-600 via-purple-600 to-indigo-600 px-6 py-5">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h3 className="text-xl font-bold text-white">Your Personalized Career Roadmap</h3>
              <p className="text-blue-100 text-sm">AI-powered guidance for your professional growth</p>
            </div>
          </div>
          <div className="flex items-center space-x-2">
            <button 
              onClick={handleShare}
              className="bg-white/20 hover:bg-white/30 backdrop-blur-sm px-4 py-2 rounded-lg text-white text-sm font-medium transition-all duration-200 flex items-center space-x-2"
            >
      ...
      {/* Content Area */}
      <div className="p-6">
        <div className="bg-gray-50/50 rounded-xl p-6 min-h-96">
        ...
        <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-gradient-to-br from-blue-50 to-blue-100 p-5 rounded-xl border border-blue-200">
            <div className="flex items-center space-x-3 mb-3">
              ...
              <h4 className="font-bold text-gray-800">Start Today</h4>
            </div>
            <p className="text-sm text-gray-600 mb-3">Begin with the most critical skills identified in your gap analysis.</p>
          ...
          <div className="bg-gradient-to-br from-green-50 to-green-100 p-5 rounded-xl border border-green-200">
            <div className="flex items-center space-x-3 mb-3">
              <span className="text-2xl">📖</span>
              <h4 className="font-bold text-gray-800">Learn & Practice</h4>
            </div>
            <p className="text-sm text-gray-600 mb-3">Access curated resources and start your first project.</p>
          ...
          <div className="bg-gradient-to-br from-purple-50 to-purple-100 p-5 rounded-xl border border-purple-200">
            <div className="flex items-center space-x-3 mb-3">
              <span className="text-2xl">🤝</span>
              <h4 className="font-bold text-gray-800">Connect & Grow</h4>
            </div>
            <p className="text-sm text-gray-600 mb-3">Join communities and start building your professional network.</p>
