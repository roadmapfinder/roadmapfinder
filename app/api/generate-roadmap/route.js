...
function selectGroqModel(query) {
  ...
  const complexKeywords = ['advanced', 'expert', 'comprehensive', 'detailed', 'full-stack', 'complete'];
...
// ============================================================================
// OPTIMIZED POST HANDLER - Maximum speed with GROQ API
// ============================================================================
export async function POST(request) {
  ...
  try {
    ...
    const [apiKey, body] = await Promise.all([
      Promise.resolve(process.env.NEXT_PUBLIC_GROQ_API_KEY), // Changed to GROQ_API_KEY
      ...
      "https://api.groq.com/openai/v1/chat/completions", // GROQ API endpoint
      {
        ...
        body: JSON.stringify({
          ...
          messages: [
            ...
            { role: "user", content: `Generate a comprehensive learning roadmap for: ${query}` }
          ],
          temperature: 0.3, // Lower for more consistent JSON output
          max_tokens: 8000, // GROQ supports high token limits
          top_p: 0.95,
    ...
    if (!aiResponse.ok) {
      ...
      return errorResponse(
        errors[aiResponse.status] || `GROQ API error: ${errorText}`, 
        aiResponse.status
      );
    }
