...
export async function POST(request) {
  try {
    ...
    if (query.length > 500) {
      return NextResponse.json(
        { success: false, error: 'Query is too long. Maximum 500 characters.' },
        { status: 400 }
      );
    }
    ...
    try {
      ...
      return NextResponse.json(
        { 
          ...
          details: process.env.NODE_ENV === 'development' ? roadmapData : undefined
        },
        { status: 500 }
      );
    }
