...
function validateRoadmap(roadmap) {
    ...
    if (roadmap.phases.length === 0) {
        return {
            valid: false,
            error: 'Roadmap must have at least one phase'
        };
    }
    for (const phase of roadmap.phases){
        if (!phase.id || !phase.title || !phase.color) {
            return {
                valid: false,
                error: 'Each phase must have id, title, and color'
            };
        }
        if (!Array.isArray(phase.sections)) {
            return {
                valid: false,
                error: 'Each phase must have sections array'
            };
        }
    }
    return {
...
async function POST(request) {
    try {
            ...
            aiResponse = await fetch('https://openrouter.ai/api/v1/chat/completions', {
                ...
                body: JSON.stringify({
                    ...
                    messages: [
                        ...
                        {
                            role: 'user',
                            content: `Create a detailed learning roadmap for: ${trimmedQuery}`
                        }
                    ],
                    temperature: 0.7,
                    max_tokens: 4000
        ...
        return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json(validatedRoadmap, {
            status: 200,
            headers: {
